...
class Paidprogram extends NotORM {
	/* 引导页 */
	...
	public function apply($uid){
		...
		if($info){
			...
			if($status==1){
				...
				$res=\PhalApi\DI()->notorm->paidprogram_apply
					->where("uid=?",$uid)
					->update(
						array(
							'status'=>0,'uptime'=>$now
			...
			$res=\PhalApi\DI()->notorm->paidprogram_apply->insert($data);
	...
	public function getPaidprogramClassList(){
		...
		//语言包
		$list=\PhalApi\DI()->notorm->paidprogram_class
	...
	public function addPaidProgram($data){
		...
		$res=\PhalApi\DI()->notorm->paidprogram->insert($data);
		return $res;
	}

	//获取付费项目详情
	public function getPaidProgramInfo($uid,$object_id){
		$info=\PhalApi\DI()->notorm->paidprogram
			->where("id=?",$object_id)
			->fetchOne();
		...
		foreach ($video_arr as $k => $v) {
			$video_arr[$k]['video_url']=\App\get_upload_path($v['video_url']);
			$video_arr[$k]['video_length_format']=\App\getSeconds($v['video_length']);
		}

		$info['videos']=$video_arr;
		$info['video_num']=\PhalApi\T("共{num}集",['num'=>count($video_arr)]);
		...
		$user_info=\App\getUserInfo($info['uid']);
	...
	public function getMyPaidProgram($uid,$p){
		...
		$list=\PhalApi\DI()->notorm->paidprogram
		...
		foreach ($list as $k => $v) {
			...
			$list[$k]['thumb_format']=\App\get_upload_path($v['thumb']);
			if($v['type']==0){
				$list[$k]['video_num']=\PhalApi\T('共{num}集',['num'=>1]);
			}else{
				$video_arr=json_decode($v['videos'],true);
				$list[$k]['video_num']=\PhalApi\T('共{num}集',['num'=>count($video_arr)]);
			}

			$list[$k]['money']=\PhalApi\T('￥').$v['money'];
	...
	public function getOrderId($orderinfo){
		...
		$result=\PhalApi\DI()->notorm->paidprogram_order->insert($orderinfo);
	...
	public function balancePay($uid,$orderinfo){
		...
		//扣除用户余额
		$res=\App\setUserBalance($uid,0,$info['money']);
		...
		if(!$result){
			...
			//返还用户余额
			\App\setUserBalance($uid,1,$info['money']);
			return -1;
		}
		...
		\App\addBalanceRecord($data);

		$money=$info['money'];

		//获取用户的抽水比例
		$apply_info=\PhalApi\DI()->notorm->paidprogram_apply->where("uid=?",$info['uid'])->fetchOne();
		...
		//给发布者增加余额
		\App\setUserBalance($info['uid'],1,$money);
	...
	public function checkIsBuy($uid,$object_id){
		...
		$info=\PhalApi\DI()->notorm->paidprogram_order
			->where("uid=? and object_id=? and status=1",$uid,$object_id)
			->fetchOne();
	...
	public function getPaidProgramList($uid,$p){
		...
		$pnums=50;
		$start=($p-1)*$pnums;
		$list=\PhalApi\DI()->notorm->paidprogram_order
				->select("object_id")
		...
		foreach ($list as $k => $v) {
			//获取付费项目详情
			$info=\PhalApi\DI()->notorm->paidprogram->select("id,uid,title,thumb,type,videos")->where("id=?",$v['object_id'])->fetchOne();
			$list[$k]['thumb']=\App\get_upload_path($info['thumb']);
			$list[$k]['title']=$info['title'];
			$user_info=\App\getUserInfo($v['touid']);
			$userinfo['avatar']=$user_info['avatar'];
			$userinfo['user_nickname']=$user_info['user_nickname'];
			...
			if($info['type']==0){
				$list[$k]['video_num']=\PhalApi\T('付费视频').'|'.\PhalApi\T('共{num}集',['num'=>1]);
			}else{
				$video_arr=json_decode($info['videos'],true);
				$count=count($video_arr);
				$list[$k]['video_num']=\PhalApi\T('付费视频').'|'.\PhalApi\T('共{num}集',['num'=>$count]);
			}
		}
	...
	public function setComment($uid,$object_id,$grade){
		...
		$res=\PhalApi\DI()->notorm->paidprogram_comment->insert($data);
	...
	public function checkIsComment($uid,$object_id){
		...
		$info=\PhalApi\DI()->notorm->paidprogram_comment
			->where("uid=? and object_id=?",$uid,$object_id)
			->fetchOne();
