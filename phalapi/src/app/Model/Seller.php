...
class Seller extends NotORM {
    ...
    public function getHome($uid){
        ...
        $wait_payment=\App\getOrderNums(['shop_uid'=>$uid,'status'=>0]);
        $wait_shipment=\App\getOrderNums(['shop_uid'=>$uid,'status'=>1]);
        $wait_refund=\App\getOrderNums(['shop_uid'=>$uid,'status'=>5,'refund_status'=>0,'refund_shop_result'=>0]);
	...
	public function getGoodsClass($uid) {
        ...
        $goods_class=\PhalApi\DI()->notorm->seller_goods_class
        ...
        //获取这些分类中有三级分类的
        $list1=\PhalApi\DI()->notorm->shop_goods_class
            ->select("gc_one_id")
            ->where("gc_grade=3")
            ->where('gc_one_id',$gc_one_ids)
            ->fetchAll();
        ...
        $class_list=\App\getcaches("goodsClass");

        //语言包
        if(!$class_list){

        	//获取一级分类
        	$class_list=\PhalApi\DI()->notorm->shop_goods_class
        	...
        	foreach ($class_list as $k => $v) {
	            ...
	            $two_list=\PhalApi\DI()->notorm->shop_goods_class
                    ->select("gc_id,gc_name,gc_name_en")
                    ->where("gc_parentid=? and gc_isshow=1",$v['gc_id'])
                    ->order("gc_sort")
                    ->fetchAll();
	            ...
	            if($two_list){
	                foreach ($two_list as $k1 => $v1) {
	                    //查询三级分类
	                    $three_list=\PhalApi\DI()->notorm->shop_goods_class
                            ->select("gc_id,gc_name,gc_name_en")
                            ->where("gc_parentid=? and gc_isshow=1",$v1['gc_id'])
                            ->order("gc_sort")
                            ->fetchAll();
        	...
        	\App\setcaches("goodsClass",$class_list);
        }
        ...
        foreach ($seller_class_list as $k => $v) {

            foreach ($v['two_list'] as $k1 => $v1) {

                if($language=="en"){
                    $v1['gc_name']=$v1['gc_name_en'];
                }

                foreach ($v1['three_list'] as $k2 => $v2) {
                    if($language=='en'){
                        $v2['gc_name']=$v2['gc_name_en'];
                    }
    ...
    public function setGoods($data){
        ...
        $uid=$data['uid'];
        $name=$data['name'];
        $exist=\PhalApi\DI()->notorm->shop_goods->where("uid=? and name=?",$uid,$name)->fetchOne();
        ...
        //向商品表写入数据
        $res=\PhalApi\DI()->notorm->shop_goods->insert($data);
        return $res;
    }
    ...
    public function getGoodsNums($uid){
        ...
        $onsale=\PhalApi\DI()->notorm->shop_goods->where("uid={$uid} and status=1")->count();
        if($onsale){
            $res['onsale']=$onsale;
        }

        $onexamine=\PhalApi\DI()->notorm->shop_goods->where("uid={$uid} and (status=0 or status=2)")->count(); //审核
        if($onexamine){
            $res['onexamine']=$onexamine;
        }

        $remove_shelves=\PhalApi\DI()->notorm->shop_goods->where("uid={$uid} and (status=-1 or status=-2)")->count();
        if($remove_shelves){
    ...
    public function getGoodsList($uid,$type,$p){
        ...
        $list=\App\handleGoodsList($where,$p);
    ...
    public function upGoodsSpecs($uid,$goodsid,$specs){
        $goodsinfo=\PhalApi\DI()->notorm->shop_goods->select("uid,specs,status")->where("id=?",$goodsid)->fetchOne();
    ...
    public function upGoods($uid,$goodsid,$data){
        ...
        $res=\PhalApi\DI()->notorm->shop_goods->where("id=?",$goodsid)->update($data);
        if($res===false){
            return 1002;
        }
    ...
    public function getExpressList(){
        ...
        //语言包
        $express_list=\PhalApi\DI()->notorm->shop_express
            ->select("id,express_name,express_name_en,express_phone,express_thumb")
    ...
    public function getGoodsOrderList($uid,$type,$p){
        ...
        //订单自动处理
        \App\goodsOrderAutoProcess($uid,array('shop_uid'=>$uid));
        ...
        foreach ($list as $k => $v) {
            ...
            switch ($v['status']) {
                ...
                case '-1':
                    $list[$k]['status_name']=\PhalApi\T('已关闭');
                    break;
                ...
                case '1':
                    ...
                    $list[$k]['status_name']=\PhalApi\T('待发货');
                    break;

                case '2':
                    
                    $list[$k]['status_name']=\PhalApi\T('已发货');
                    break;

                case '3':
                    
                    $list[$k]['status_name']=\PhalApi\T('已签收');
                    break;
                ...
                case '5':
                    ...
                    if($v['refund_status']==0){
                        $list[$k]['status_name']=\PhalApi\T('退款中');

                        if($v['refund_shop_result']==-1){
                            $list[$k]['status_name']=\PhalApi\T('已拒绝');
                        }
            ...
            //判断卖家是否关注了买家
            $isattention=\App\isAttention($uid,$v['uid']);
            ...
            if($v['status']==5){
                //获取订单退款类型
                $type=\PhalApi\DI()->notorm->shop_order_refund->where("orderid={$v['id']}")->fetchOne("type");
                if($type){
                    $refund_type=$type;
                }
            }
    ...
    public function getTypeListNums($uid,$type_arr){
        ...
        $type_list_nums['all_nums']=\App\getOrderNums($all_where);
        ...
        foreach ($type_arr as $k => $v) {
            ...
            switch ($v) {
                ...
                case 'wait_payment': //待付款
                    ...
                    $type_list_nums['wait_payment_nums']=\App\getOrderNums($where);
                ...
                case 'wait_shipment': //待返货
                    ...
                    $type_list_nums['wait_shipment_nums']=\App\getOrderNums($where1);
    ...
    public function getRefundRefuseReason(){
        ...
        //语言包
        $list=\PhalApi\DI()->notorm->shop_refuse_reason
    ...
    public function getWaitSettlementTotal($uid){
        $sum=\PhalApi\DI()->notorm->shop_order
    ...
    public function upOutsideGoods($goodsid,$data){
        ...
        $uid=$data['uid'];
        $name=$data['name'];
        $exist=\PhalApi\DI()->notorm->shop_goods->where("uid=? and name=? and id !=?",$uid,$name,$goodsid)->fetchOne();
    ...
    public function setPlatformGoods($uid,$goodsid){
        ...
        if($info){
          ...
          if(!$res){
           ...
           $res=\PhalApi\DI()->notorm->seller_platform_goods->insert($data);
           if(!$res){
                return 1002;
           }
