...
class Login extends NotORM {
    ...
    public function userReg($country_code,$user_login,$source) {
		...
		if($info){
			$uid=$info['id'];
			$token=md5(md5($uid.$user_login.time()));
			$this->updateToken($uid,$token,$source);
        ...
        if($reg_reward>0){
	        ...
	        $insert = array('type' => '1', 'action' => '5', 'uid' => $uid, 'totalcoin' => $reg_reward, 'showid' => 0, 'addtime' => time());
	    ...
	    if ($isexist['coin']== 0){
		    $info['coin'] = $coinNums;
		    $rs = \PhalApi\DI()->notorm->user->where('id = ?', $uid)->update(['coin' => $coinNums]);
		    if(!$rs) {
			    return 1009;
		    }
		}
	...
	/* 更新token 登陆信息 */
    public function updateToken($uid,$token,$data="pc") {
        $nowtime=time();
        ...
        if(!$isok){
			...
			->insert(array("user_id"=>$uid,"token"=>$token, "expire_time"=>$expiretime ,'create_time' => $nowtime,'device_type' =>$data));
        }else{
			\PhalApi\DI()->notorm->user_token
		        ->where('user_id=?', $uid)
		        ->update(array('token' => $token, 'expire_time' => $expiretime, 'create_time' => $nowtime, 'device_type' => $data));
        }
	...
	public function findMobile($country_code,$mobile)
	{
		$isexist=\PhalApi\DI()->notorm->user
			->select('id')
	...
	public function findUserInfo($country_code, $user_login)
	{
		$info = \PhalApi\DI()->notorm->user
			->select($this->fields . ',user_pass')
			->where('country_code=? and user_login=? and user_type="2"', $country_code, $user_login)
...
//		$info['level_anchor'] = \App\getLevelAnchor($info['votestotal']);
	...
	public function addUserInfo($country_code, $user_login,$source)
	{
		$configpri = \App\getConfigPri();
		$reg_reward = $configpri['reg_reward'];
        do {
			...
			// 执行数据库查询，检查是否已有该随机数
			$find = \PhalApi\DI()->notorm->user
				->where('rand_id = ?', $randomNumber)
				->fetchOne();
		...
		if ($reg_reward > 0) {
			$insert = array('type' => '1', 'action' => '11', 'uid' => $uid, 'touid' => $uid, 'giftid' => 0, 'giftcount' => 1, 'totalcoin' => $reg_reward, 'showid' => 0, 'addtime' => time());
			\PhalApi\DI()->notorm->user_coinrecord->insert($insert);
			$insert = array('type' => '1', 'action' => '5', 'uid' => $uid, 'showid' => 0, 'addtime' => time());
			\PhalApi\DI()->notorm->user_coinrecord_all->insert($insert);
		}
		$code = $this->createCode();
		$code_info = array('uid' => $uid, 'code' => $code);
