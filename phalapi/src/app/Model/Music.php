...
class Music extends NotORM {
	...
	public function classify_list(){
		$list=\PhalApi\DI()->notorm->music_classify
			->select("id,title,title_en,img_url")
			->where("isdel=0")
	...
	public function music_list($classify,$uid,$p){
		...
		$list=\PhalApi\DI()->notorm->music
			->select("id,title,author,img_url,length,file_url,use_nums")
			->where($where)
			->order("use_nums desc")
			->limit($start,$nums)
			->fetchAll();
		...
		if($uid<1){ //游客
			...
			foreach ($list as $k => $v) {
				$list[$k]['iscollect']=0;
				$list[$k]['img_url']=\App\get_upload_path($v['img_url']);
				$list[$k]['file_url']=\App\get_upload_path($v['file_url']);
			}
		}else{

			
			//获取本人收藏列表
			$collectLists=\PhalApi\DI()->notorm->music_collection
				->select("music_id")
	...
	public function collectMusic($uid,$musicid){
		...
		if($isexist){
			...
			if($isexist['status']==1){ //已收藏
				//将状态改为取消收藏
				$result=\PhalApi\DI()->notorm->music_collection
					->where("uid=? and music_id=?",$uid,$musicid)
					->update(
						array(
							"status"=>0,
							"updatetime"=>time()
			...
			$result=\PhalApi\DI()->notorm->music_collection->insert($data);
			if($result!==false){
				return 300;
			}else{
				return 301;
	...
	public function getCollectMusicLists($uid,$p){
		...
		foreach ($list as $k => $v) {
			$musicinfo=\PhalApi\DI()->notorm->music
				->select("title,author,img_url,length,file_url,use_nums")
				->where("id=?",$v['music_id'])
				->fetchOne();

			unset($list[$k]['addtime']);
			$list[$k]['collecttime']=\App\datetime($v['addtime']); //收藏时间
			$list[$k]['title']=$musicinfo['title'];
			$list[$k]['author']=$musicinfo['author'];
			$list[$k]['img_url']=\App\get_upload_path($musicinfo['img_url']);
			$list[$k]['length']=$musicinfo['length'];
			$list[$k]['file_url']=\App\get_upload_path($musicinfo['file_url']);
