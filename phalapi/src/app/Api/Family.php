...
use PhalApi\Api;
use App\Domain\Family as Domain_Family;
/**
 * 家族
 */
class Family extends Api {
	...
	public function createFamily() {
		$rs = array('code' => 0, 'msg' => \PhalApi\T('家族创建成功'), 'info' => array());
		
		$uid=\App\checkNull($this->uid);
        $token=\App\checkNull($this->token);
        $familyname=\App\checkNull($this->familyname);
        $username=\App\checkNull($this->username);
        $cardno=\App\checkNull($this->cardno);
        $divide_family=\App\checkNull($this->divide_family);
        $briefing=\App\checkNull($this->briefing);
        $apply_pos=\App\checkNull($this->apply_pos);
        $apply_side=\App\checkNull($this->apply_side);
        $apply_map=\App\checkNull($this->apply_map);
        ...
        if(!$familyname){
        	$rs['code']=1001;
        	$rs['msg']=\PhalApi\T('请填写家族名称');
        	return $rs;
        }

        if(mb_strlen($familyname)>15){
        	$rs['code']=1001;
        	$rs['msg']=\PhalApi\T('家族名称在15个字符以内');
        	return $rs;
        }
        ...
        $cardno_check=\App\isCreditNo($cardno);
        if(!$cardno_check){
        	$rs['code']=1004;
        ...
        if(!is_numeric($divide_family)){
        	$rs['code']=1004;
        	$rs['msg']=\PhalApi\T('抽成比例必须是数字');
        	return $rs;
        }

        if($divide_family<0 || $divide_family>100 || floor($divide_family)!=$divide_family){
        	$rs['code']=1004;
        	$rs['msg']=\PhalApi\T('抽成比例必须是0-100之间的整数');
        	return $rs;
        }

        if(!$briefing){
        	$rs['code']=1005;
        	$rs['msg']=\PhalApi\T('请填写家族简介');
        	return $rs;
        }
        if(mb_strlen($briefing)>150){
        	$rs['code']=1005;
        	$rs['msg']=\PhalApi\T('家族简介应在150字符以内');
        	return $rs;
        }

        if(!$apply_pos){
        	$rs['code']=1006;
        	$rs['msg']=\PhalApi\T('请上传身份证正面照');
        	return $rs;
        }

        if(!$apply_side){
        	$rs['code']=1007;
        	$rs['msg']=\PhalApi\T('请上传身份证背面照');
        	return $rs;
        }

        if(!$apply_map){
        	$rs['code']=1008;
        	$rs['msg']=\PhalApi\T('请上传家族图标');
        	return $rs;
        }
		...
		if($res==1001){
			$rs['code']=1009;
			$rs['msg']=\PhalApi\T('家族创建失败');
			return $rs;		
		}

		if($res==1002){
			$rs['code']=1010;
			$rs['msg']=\PhalApi\T('您提交的家族申请正在审核中');
			return $rs;		
		}

		if($res==1003){
			$rs['code']=1011;
			$rs['msg']=\PhalApi\T('家族已创建成功');
			return $rs;		
		}
