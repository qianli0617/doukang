...
class VideoController extends AdminbaseController {
    ...
    public function setDel(){
        ...
        if($id){
            ...
            if($result){
			    ...
			    setAdminLog($action);
				if ($isdel==1){
					$findVideo = DB::name('video')->where(['id' => $id])->find();
					$content = "您的视频《{$findVideo['title']}》被下架";
					$content_en = "Your video {$findVideo['title']} has been removed from the shelves";
	                $this->videoRemoved($content, $content_en, $findVideo['uid']);
				}
	...
	public function videoRemoved($content, $content_en, $touid)
	{
		...
		//写入记录 // 3 视频下架消息
		$id = addSysytemInfo($send_user, $content, $content_en, 3);
		if (!$id) {
			$this->error('推送失败！');
		}
