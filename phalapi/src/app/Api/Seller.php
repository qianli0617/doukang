...
use PhalApi\Api;
use App\Domain\Seller as Domain_Seller;
...
class Seller extends Api {
	...
	public function getHome() {
		...
		if($apply_status==2){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('店铺认证被拒绝,请重新认证');
			return $rs;
		}
	...
	public function setGoods(){
		...
		$rs = array('code' => 0, 'msg' => \PhalApi\T('商品发布成功'), 'info' => array());
		
        $uid=\App\checkNull($this->uid);
        $token=\App\checkNull($this->token);
        $one_classid=\App\checkNull($this->one_classid);
        $two_classid=\App\checkNull($this->two_classid);
        ...
        $pictures=\App\checkNull($this->pictures);
        $specs=$this->specs;
        $postage=\App\checkNull($this->postage);
        $share_income=\App\checkNull($this->share_income);
		...
		if(!$one_classid){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('请选择商品一级分类');
			return $rs;
		}

		if(!$two_classid){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('请选择商品二级分类');
			return $rs;
		}

		if(!$three_classid){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('请选择商品三级分类');
			return $rs;
		}

		if(!$name){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('请填写商品名称');
			return $rs;
		}

		if(mb_strlen($name)>15){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('商品名称不能超过{num}字',['num'=>15]);
			return $rs;
		}

		if(!$thumbs){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('请上传商品缩略图');
			return $rs;
		}

		if(!$content){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('请添加商品详情');
			return $rs;
		}

		if(mb_strlen($content)>300){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('商品详情不能超过{num}字',['num'=>300]);
			return $rs;
		}

		if(!$specs){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('请添加商品规格');
			return $rs;
		}
		...
		if($specname_false){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('请填写商品规格名称');
			return $rs;
		}

		if($specname_len_false){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('商品规格名称应在{num}个字符以内',['num'=>15]);
			return $rs;
		}

		if($specnum_false){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('商品规格库存在{num1}-{num2}之间',['num1'=>1,'num2'=>9999999]);
			return $rs;
		}

		if($specprice_false){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('商品规格单价在{num1}-{num2}之间',['num1'=>1,'num2'=>99999999]);
			return $rs;
		}

		if($specthumb_false){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('请上传商品规格封面图');
			return $rs;
		}

		if($postage<0||$postage>99999){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('商品运费在{num1}-{num2}之间',['num1'=>0,'num2'=>99999]);
			return $rs;
		}

        if($share_income<0||$share_income>99999){
            $rs['code']=1001;
            $rs['msg']=\PhalApi\T('分享佣金在{num1}-{num2}之间',['num1'=>0,'num2'=>99999]);
            return $rs;
        }

        $num_check=\App\checkNumber($share_income);

        if(!$num_check){
            $rs['code']=1001;
            $rs['msg']=\PhalApi\T('请确认分享佣金');
            return $rs;
        }

        if($share_false){
            $rs['code']=1001;
            $rs['msg']=\PhalApi\T('分享佣金应低于商品价格');
            return $rs;
        }
		...
		if($res==1001){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('您已经发布过同名商品');
			return $rs;
		}else if(!$res){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('商品发布失败');
			return $rs;
		}
	...
	public function upReceiverAddress(){
		$rs = array('code' => 0, 'msg' => \PhalApi\T('退货地址修改成功'), 'info' => array());
	...
	public function upGoodsSpecs(){
		$rs = array('code' => 0, 'msg' => \PhalApi\T('商品规格更新成功'), 'info' => array());
		...
		if($specprice_false){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('商品规格单价在{num1}-{num2}之间',['num1'=>1,'num2'=>99999]);
			return $rs;
		}

		if($postage<0||$postage>999){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('商品规格运费在{num1}-{num2}之间',['num1'=>0,'num2'=>999]);
			return $rs;
		}
		...
		if($res==1003){
			$rs['code']=1003;
			$rs['msg']=\PhalApi\T('商品审核中,无法修改规格');
			return $rs;
		}

		if($res==1004){
			$rs['code']=1004;
			$rs['msg']=\PhalApi\T('商品下架,无法修改规格');
			return $rs;
		}

		if($res==-1){
			$rs['code']=1005;
			$rs['msg']=\PhalApi\T('商品规格更新失败');
			return $rs;
		}
	...
	public function upGoods(){
		$rs = array('code' => 0, 'msg' => \PhalApi\T('商品修改成功'), 'info' => array());
		...
		if($postage<0||$postage>999){
			$rs['code']=1001;
			$rs['msg']=\PhalApi\T('商品运费在{num1}-{num2}之间',['num1'=>0,'num2'=>999]);
			return $rs;
		}

        if($share_income<0||$share_income>999){
            $rs['code']=1001;
            $rs['msg']=\PhalApi\T('分享佣金在{num1}-{num2}之间',['num1'=>0,'num2'=>999]);
            return $rs;
        }
	...
	public function delGoods() {
		$rs = array('code' => 0, 'msg' => \PhalApi\T('商品删除成功'), 'info' => array());
	...
	public function setExpressInfo(){
		$rs=array('code'=>0,'msg'=>\PhalApi\T('订单发货成功'),'info'=>array());
		$uid=\App\checkNull($this->uid);
		$token=\App\checkNull($this->token);
		$orderid=\App\checkNull($this->orderid);
		$expressid=\App\checkNull($this->expressid);
		$express_number=\App\checkNull($this->express_number);
		$time=\App\checkNull($this->time);
		$sign=\App\checkNull($this->sign);
		...
		$order_info=\App\getShopOrderInfo(['shop_uid'=>$uid,'id'=>$orderid]);
		if(!$order_info){
			$rs['code'] = 1001;
    ...
    public function getGoodsOrderInfo(){
        ...
        $order_info=\App\handleGoodsOrder($order_info);


        if($order_info['status']==1){ //订单待发货
        	$order_info['status_name']=\PhalApi\T('待发货');
        	$order_info['status_desc']=\PhalApi\T('请尽快发货,逾期将会自动关闭订单,钱款会退还给买家');
        }

        if($order_info['status']==2){ //订单已发货
        	$order_info['status_name']=\PhalApi\T('已发货');
        }

        if($order_info['status']==3&&$order_info['settlement_time']==''){ //已签收 未结算
        	$order_info['status_name']=\PhalApi\T('已签收');
        	$order_info['status_desc']=\PhalApi\T('商品钱款会尽快核算发放');
        }

        if($order_info['status']==3&&$order_info['settlement_time']!=''){ //已签收 已结算
        	$order_info['status_name']=\PhalApi\T('交易成功');
        	$order_info['status_desc']=\PhalApi\T('商品钱款已发放到您的账户中');
        }

        $rs['info'][0]['order_info']=$order_info;

        $model=new Domain_Shop();
        $shop_info=$model->getShop($order_info['shop_uid']);

        //判断店铺用户是否关注了订单用户
        $isattention=\App\isAttention($uid,$order_info['uid']);
    ...
    public function getGoodsOrderRefundInfo(){
        ...
        if($refund_info['status']==-1){ //买家取消
            ...
            $status_name=\PhalApi\T('买家已取消退款申请');
        	...
        	if($is_platform_interpose==0){ //平台未介入
        		...
        		if($refund_shop_result==0){
	                ...
	                if($shop_process_num>0){
	                	$status_name=\PhalApi\T('买家重新申请退款');
	                }

	            }elseif($refund_shop_result==-1){ //卖家拒绝

	                $status_name=\PhalApi\T('您已经拒绝了买家退款申请{num}次',['num'=>$shop_process_num]);
	                if($shop_process_num<3){ 
	                	$status_desc=\PhalApi\T('买家可重新提交申请或申请平台介入');
	                	$can_handle='0';

	                }else{
	                	$status_desc=\PhalApi\T('买家可申请平台介入');
	                }
            ...
            if($is_platform_interpose){ //平台介入
                ...
                if($refund_platform_result==-1){ //拒绝
                    $status_name=\PhalApi\T('平台拒绝了买家的退款申请');
                    $status_time=date("Y-m-d H:i:s",$refund_info['platform_process_time']); //平台处理时间
                }elseif($refund_platform_result==1){
                    $status_name=\PhalApi\T('平台同意了买家的退款申请');
                    $status_time=date("Y-m-d H:i:s",$refund_info['platform_process_time']); //平台处理时间
                    $status_desc=\PhalApi\T('如有疑问请与平台协商');
                }
                ...
                if($refund_shop_result==1){ //卖家同意
                   ...
                   $status_desc=\PhalApi\T('您已同意退款,退款金额将退回到买家账户余额');
                }elseif($refund_shop_result==-1){ //卖家拒绝
                    $status_name=\PhalApi\T('您已经拒绝了买家退款申请');
                    $status_time=date("Y-m-d H:i:s",$refund_info['system_process_time']); //系统自动处理时间
                    $status_desc=\PhalApi\T('您已拒绝退款,拒绝原因:');
                }
        ...
        $buyer_info=\App\getUserInfo($refund_info['uid']);
    ...
    public function setGoodsOrderRefund(){
    	$rs=array('code'=>0,'msg'=>\PhalApi\T('退款处理完成'),'info'=>array());
        ...
        $refuse_desc=\App\checkNull($this->refuse_desc);
        $time=\App\checkNull($this->time);
        $sign=\App\checkNull($this->sign);
        ...
        if($type==0){ //拒绝
        	...
        	if($refuse_desc){
        		if(mb_strlen($refuse_desc)>300){
        			$rs['code']=1001;
		            $rs['msg']=\PhalApi\T('拒绝详细原因应在{num}字以内',['num'=>300]);
		            return $rs;
        		}
        	}
        ...
        if($refund_status==1){ //已经完成
        	...
        	if($is_platform_interpose==1){ //平台介入
        		...
        		if($platform_result==1){ //平台同意
        			$rs['code']=1001;
		            $rs['msg']=\PhalApi\T('平台已同意退款');
		            return $rs;

        		}elseif($is_platform_interpose==-1){ //平台拒绝
        			$rs['code']=1001;
		            $rs['msg']=\PhalApi\T('平台已拒绝退款');
		            return $rs;
        		}
        		...
        		if($shop_result==1){ //卖家同意
        			$rs['code']=1001;
		            $rs['msg']=\PhalApi\T('您已同意退款');
		            return $rs;
        		}elseif($shop_result==-1){
        			$rs['code']=1001;
		            $rs['msg']=\PhalApi\T('您已拒绝退款');
		            return $rs;
        		}
        	...
        	if($shop_result==-1){
        		$rs['code']=1001;
	            $rs['msg']=\PhalApi\T('您已经拒绝,不能再操作');
	            return $rs;
        	}elseif($shop_result==1){
        		$rs['code']=1001;
	            $rs['msg']=\PhalApi\T('您已经同意,不能再操作');
	            return $rs;
        	}

        	if($shop_process_num>=3){
        		$rs['code']=1001;
	            $rs['msg']=\PhalApi\T('您已经拒绝{num}次,不能再操作',['num'=>$shop_process_num]);
	            return $rs;
        	}
        ...
        if($type==1){ //卖家同意
        	...
        	//给买家退钱
        	\App\setUserBalance($order_info['uid'],1,$order_info['total']);
            ...
            \App\addBalanceRecord($data2);

            //减去商品销量
            \App\changeShopGoodsSaleNums($order_info['goodsid'],0,$order_info['nums']);

            //减去店铺销量
        	\App\changeShopSaleNums($order_info['shop_uid'],0,$order_info['nums']);
            ...
            \App\txMessageIM(json_encode($im_msg),$order_info['uid'],'goodsorder_admin','TIMCustomElem');
        	...
        	$refund_history_data=array(
            	...
            	'addtime'=>$now,
            	'desc'=>'卖家拒绝退款',
                'desc_en'=>'Seller refuses to refund',
            	'refuse_reason'=>$refuse_reason,
                'refuse_reason_en'=>$refuse_reason_en,
    ...
    public function setOutsideGoods(){
        ...
        $href=$this->href;
        $name=\App\checkNull($this->name);
        $original_price=\App\checkNull($this->original_price);
        $present_price=\App\checkNull($this->present_price);
        $goods_desc=\App\checkNull($this->goods_desc);
        ...
        if(mb_strlen(trim($name))>20){
        	$rs['code']=1001;
            $rs['msg']=\PhalApi\T('商品名称不能超过{num}字',['num'=>20]);
            return $rs;
        }

        if(!$original_price){
        	$rs['code']=1001;
            $rs['msg']=\PhalApi\T('请填写商品原价');
            return $rs;
        }

        if(!is_numeric($original_price)){
        	$rs['code']=1001;
            $rs['msg']=\PhalApi\T('商品原价必须为数字');
            return $rs;
        }

        if($original_price>99999999 || $original_price<=0){
            $rs['code']=1001;
            $rs['msg']=\PhalApi\T('商品原价必须在{num1}-{num2}之间',['num1'=>0,'num2'=>99999999]);
            return $rs;
        }

        if(!$present_price){
        	$rs['code']=1001;
            $rs['msg']=\PhalApi\T('请填写商品现价');
            return $rs;
        }

        if(!is_numeric($present_price)){
        	$rs['code']=1001;
            $rs['msg']=\PhalApi\T('商品现价必须为数字');
            return $rs;
        }

        if($present_price>99999999 || $present_price<=0){
            $rs['code']=1001;
            $rs['msg']=\PhalApi\T('商品现价必须在{num1}-{num2}之间',['num1'=>0,'num2'=>99999999]);
            return $rs;
        }

        if(!$goods_desc){
        	$rs['code']=1001;
            $rs['msg']=\PhalApi\T('请填写商品简介');
            return $rs;
        }

        if(mb_strlen(trim($goods_desc))>50){
        	$rs['code']=1001;
            $rs['msg']=\PhalApi\T('商品简介必须在{num}字以内');
            return $rs;
        }

        if(!$thumb){
        	$rs['code']=1001;
            $rs['msg']=\PhalApi\T('请上传商品图片');
            return $rs;
        }
    ...
    public function upOutsideGoods(){
        ...
        if(mb_strlen(trim($goods_desc))>50){
            $rs['code']=1001;
            $rs['msg']=\PhalApi\T('商品简介必须在{num}字以内',['num'=>50]);
            return $rs;
        }
    ...
    public function setPlatformGoods(){
		...
		if($res==1001){
			$rs['code']=1003;
			$rs['msg']=\PhalApi\T('商品取消代售失败');
			return $rs;
		}

		if($res==1002){
			$rs['code']=1004;
			$rs['msg']=\PhalApi\T('商品添加失败');
			return $rs;
		}
