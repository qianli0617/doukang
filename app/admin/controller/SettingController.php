...
//            }
    ...
    public function configpriPost(){
        ...
        if ($this->request->isPost()) {
	        ...
	        if (!is_numeric($options['new_coin_nums'])) {
		        $this->error('新用户奖励抖康币必须为数字');
	        }
	
			if ($options['new_coin_nums'] <= 0){
				$this->error('新用户奖励抖康币不能小于0个');
			}
	        ...
	        if($options['new_coin_nums'] > 0){
		        $action.='新用户奖励抖康币修改：'.$options['new_coin_nums'];
	        }
	...
	public function sensitiveWords($key='',$info=[]){
		$allInfo = cmf_get_option('configpri');
		$this->assign('sensitive_words', $allInfo['sensitive_words']);
		return $this->fetch();
	}
	public function sensitiveWordsPost(){
		if ($this->request->isPost()) {
			...
			$oldconfigpri['sensitive_words'] = $options['sensitive_words'];
			
			cmf_set_option('configpri', $oldconfigpri, true);
			$this->resetcache('getConfigPri', $oldconfigpri);

			setcaches('sensitive_words', explode(',', $oldconfigpri['sensitive_words']));
